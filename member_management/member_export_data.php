...
while ($row = mysql_fetch_object($res)) {
    ...
    else
    {
        ...
        {
            $resgen = mysql_query("SELECT * FROM bimapoa_members where policy_no='".$row->policy_no."' and relation='SELF'");
            $fetchresgen = mysql_fetch_array($resgen);
            if($fetchresgen['sex']=='Male')
            {
        ...
        else if($row->relation == 'Daughter')
        {
            $gender = 'Female';
        }
        
        xlsWriteLabel($xlsRow, 8, $gender);
        xlsWriteLabel($xlsRow, 9, $row->relation);
        xlsWriteLabel($xlsRow, 10, '');
        xlsWriteLabel($xlsRow, 11, '');
        xlsWriteLabel($xlsRow, 12, '');
        xlsWriteLabel($xlsRow, 13, '');
        xlsWriteLabel($xlsRow, 14, $row->selected_provider_code);
        xlsWriteLabel($xlsRow, 15, '');
        xlsWriteLabel($xlsRow, 16, '');
        xlsWriteLabel($xlsRow, 17, $row->date_captured);
        xlsWriteLabel($xlsRow, 18, '');
        xlsWriteLabel($xlsRow, 19, '');
        xlsWriteLabel($xlsRow, 20, '');
        xlsWriteLabel($xlsRow, 21, '');
        $xlsRow++;   
    }
}
xlsEOF();
