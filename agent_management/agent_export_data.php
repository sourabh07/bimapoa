...
$res = mysql_query("SELECT * FROM agent");
function xlsBOF()
{
...
header("Content-Disposition: attachment;filename=BIMA-POA Agent Data.xls");
header("Content-Transfer-Encoding: binary ");
xlsBOF();
//xlsWriteLabel(0,0,"S.No");
xlsWriteLabel(0,0,"agent_id");
xlsWriteLabel(0,1,"name");
xlsWriteLabel(0,2,"date_created");
xlsWriteLabel(0,3,"agent_code");
//xlsWriteLabel(0,4,"phone_number");
...
while($row = mysql_fetch_object($res)) {
	...
	xlsWriteLabel($xlsRow,0,$row->agent_id);
	xlsWriteLabel($xlsRow,1,$row->name);
	xlsWriteLabel($xlsRow,2,$row->date_created);
	xlsWriteLabel($xlsRow,3,$row->agent_code);
	//xlsWriteLabel($xlsRow,4,$row->phone_number);
