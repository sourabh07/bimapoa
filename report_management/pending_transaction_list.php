...
<ul>
	...
	<li><a href="#">Manage Pending Installments</a></li>
</ul>
...
<div class="box-title">
<h3><i class="icon-table"></i> M-Pesa Pending Installments Details</h3>


<button class="btn btn-mini btn-warning pull-right"
	style="margin-right: 10px;"
	onclick="window.location.href = 'pending_export_data.php'">Export CSV</button>
...
<table
	...
	<tbody>
	<?php
	//SELECT SUM(mpesa_amount) AS total FROM mpesa where mpesa_account = 'APN0001277';
	$i = 1;
	$total_amount_paid = 0;
	...
	while ($row_member = mysql_fetch_array($result_mpesa)) {
		...
		$query_total = "SELECT SUM(mpesa_amount) AS total_amount FROM mpesa where mpesa_account =  '" . $row_member['mpesa_account'] . "'";
		$result_mpesa_amount = mysql_query($query_total);
		while ($row_total_amount = mysql_fetch_array($result_mpesa_amount))
		{
			$total_amount_paid =$row_total_amount['total_amount'];
		}
		$count = $memberobj->row_count('mpesa', "mpesa_account = '" . $row_member['mpesa_account'] . "'");
		//SELECT * FROM `mpesa` WHERE `mpesa_account` = 'APN0001277' ORDER BY `mpesa_transaction_date` ASC;
		$result_date = mysql_query("select * from mpesa where mpesa_account='" . $row_member['mpesa_account'] . "' ORDER BY `mpesa_transaction_date` DESC LIMIT 1");
		$row_date = mysql_fetch_array($result_date);
			
		if ($row_member['mpesa_amount'] != 8000) {

			if (($row_member['mpesa_amount'] >= 4000 && $row_member['mpesa_amount'] < 8000 && $count == 1) || ($row_member['mpesa_amount'] == 800 && $count != 12)) {

				if ($row_member['mpesa_amount'] >= 4000 && $row_member['mpesa_amount'] < 8000) {
					$format = 'Y-m-d';
					$due_date = $count * 90;
					$newDateTermination = date( $format, strtotime ( '+'.$due_date.'day' . $row_member_name['contract_effective_date'] ) );
					//$amountRemaining = 8000 - (4000 * ($count));
					$amountRemaining = 8000 - ($total_amount_paid);
					$installmentsRemaining = 2 - $count;
				} elseif ($row_member['mpesa_amount'] >= 800 && $row_member['mpesa_amount'] <= 1600) {
					$format = 'Y-m-d';
					$due_date = $count * 30;
					$newDateTermination = date( $format, strtotime ('+'.$due_date.'day'. $row_member_name['contract_effective_date'] ) );
					//$amountRemaining = 9600 - (800 * ($count));
					$amountRemaining = 9600 - ($total_amount_paid);
					$installmentsRemaining = 12 - $count;
				}
				/*elseif($row_member['mpesa_amount'] >= 4000 && $row_member['mpesa_amount'] < 8000)
				 {
					$format = 'Y-m-d';
					$due_date = $count * 30;
					$newDateTermination = date( $format, strtotime ('+'.$due_date.'day'. $row_member_name['contract_effective_date'] ) );
					$amountRemaining = 8000 - ($row_member['mpesa_amount'] * ($count));
					$installmentsRemaining = 2 - $count;
					}
					elseif($row_member['mpesa_amount'] >= 800 && $row_member['mpesa_amount'] <= 1600)
					{
					...
					$due_date = $count * 30;
					$newDateTermination = date( $format, strtotime ('+'.$due_date.'day'. $row_member_name['contract_effective_date'] ) );
					$amountRemaining = 9600 - ($row_member['mpesa_amount'] * ($count));
					$installmentsRemaining = 12 - $count;
					}*/

				?>
		<tr <?php if($row_member_name['status']=='T'){ ?> style="color: red;"
				<?php }?>>
			<td class='hidden-1024'><?php echo $i ?></td>
			<td class='hidden-1024'><?php echo $amountRemaining; ?></td>
			<td class='hidden-1024'><?php echo $installmentsRemaining ?></td>
			<td class='hidden-1024'><?php echo $row_member['mpesa_account'] ?></td>
			...
			<td class='hidden-1024'><?php echo $row_date['mpesa_transaction_date'] ?>
			</td>
			<td class='hidden-1024'><?php echo $newDateTermination;?></td>
			<td class='hidden-1024'><?php echo $row_member_name['membership_no'] ?>
			</td>
			<td class='hidden-1024'><?php echo $row_member_name['membership_name'] ?>
			</td>
			<td class='hidden-1024'><?php echo $row_member_name['chw_code'] ?></td>
